...
class Pool
{
    ...
    {
        // 2分钟检测一次连接
        swoole_timer_tick( 1000 * 60 * 2 , function(){
            // 维持连接
...
$server->on("request",function($request,$response) {
    try
    {
        $pool = Pool::getInstance()->query("select * from sys_user", function($res) use($response) {
            $response->end(json_encode($res));
        });
    }
    catch (Exception $e)
    {
        error_log($e->getMessage(),3,__DIR__.'/logs/swoole-myql-pool.log');
    }
